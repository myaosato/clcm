(defpackage :clcm/inlines/entities-data
  (:use :cl)
  (:export :*entities-map*))
(in-package :clcm/inlines/entities-data)
...
(defvar *entities-map*
  (let ((map (make-hash-table :test 'equal :size (length *entities*))))
    (loop :for e :in *entities*
          :do (setf (gethash (second e) map)
                    (if (consp (first e))
                        (format nil "~A~A" (code-char (car (first e))) (code-char (cdr (first e))))
                        (format nil "~A" (code-char (first e))))))
    map))
